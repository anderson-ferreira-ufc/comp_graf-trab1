...
/* Implementem essas variáveis das trajetórias onde são
chamadas através de um <select>. Coloquem também a opção
de mostrar todas as trajetórias de uma vez ou nenhuma
*/
...
function animate() {
    ...
    // Descomente a linha que começa com traj e verifique o efeito.
    ...
    window.addEventListener('keydown', (event) => {
        switch (event.key) {
            case 'm':
                animationSpeed += 1; // aumenta velocidade
                document.getElementById("velocidade").innerHTML = animationSpeed; // mostrar na tela velocidade atual
                break;
            case 'l':
                if (animationSpeed > 1) { // evita velocidade menor que 1
                    animationSpeed -= 1; // diminui velocidade
                    document.getElementById("velocidade").innerHTML = animationSpeed;
                }
                break;
            case ' ':
