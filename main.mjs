...
window.addEventListener('keydown', (event) => {
    switch (event.key) {
        case '+':
            if (aux < 10) {
                animationSpeed *= 1.1; // aumenta velocidade
                aux += 1;
                document.getElementById("velocidade").innerHTML = aux; // mostrar na tela velocidade atual
            }
            break;
        case '-':
            if (aux > 1) { // evita velocidade menor que 0
                animationSpeed /= 1.1; // diminui velocidade
                aux -= 1;
        ...
        case ' ':
            ...
            break;
        case 'p':
            if (aux == 0) { // se ja estiver pausado despausa e volta ao inicio
