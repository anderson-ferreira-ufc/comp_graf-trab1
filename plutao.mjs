...
export const plu_sol_dist =
  (esc_dist * (5906400000 + 696340 + 1188)) / (149600000 + 696340 + 6378); // 2358.11 = Distância entre os centros de Plutão e Sol.
...
export function plutaoTranslacao() {
  ...
  const material_plu = new THREE.MeshPhongMaterial({ map: plu_loader });
  const plutao = new THREE.Mesh(geo_plu, material_plu);
  plutao.position.set(0, 0, 0);

  plutao.castShadow = true;
  plutao.receiveShadow = true;
...
export function trajetoriaPlutao() {
  ...
  const material = new THREE.LineBasicMaterial({ color: 0x8400ff });
